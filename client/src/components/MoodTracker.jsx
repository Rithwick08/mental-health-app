...
export default function MoodTracker() {
  ...
  return (
    <div className="card p-4 h-100">
      ...
      <p style={{ color: 'var(--text-light)', fontSize: '0.85rem', marginBottom: '1.25rem' }}>Tap to log your mood</p>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        {MOODS.map((m) => (
          ...
          >
            ...
            <span style={{ fontSize: '0.7rem', color: 'var(--text-medium)', marginTop: '0.2rem', fontWeight: 600 }}>{m.label}</span>
          </button>
        ))}
      </div>
      ...
      {/* History */}
      <div>
        <p style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.5rem' }}>
          Recent
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', maxHeight: '130px', overflowY: 'auto' }}>
          {moods.slice(0, 6).map((m) => {
            const found = MOODS.find(x => x.value === m.mood.toLowerCase()) || { emoji: '💭', color: '#ccc' };
            return (
              <div key={m._id} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.4rem 0.6rem', background: 'var(--cream)', borderRadius: 'var(--radius-sm)' }}>
                <span style={{ fontSize: '1.1rem' }}>{found.emoji}</span>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-medium)', fontWeight: 600 }}>{m.mood}</span>
