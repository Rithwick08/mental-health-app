...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div style={{ background: 'var(--white)', border: '1px solid var(--sage-mist)', borderRadius: 'var(--radius-sm)', padding: '0.5rem 0.75rem', fontSize: '0.85rem', color: 'var(--text-dark)', boxShadow: 'var(--shadow-sm)' }}>
        <span style={{ fontSize: '1.2rem' }}>{moodLabels[val] || '💭'}</span>
        <span style={{ marginLeft: '0.4rem', fontWeight: 600, color: 'var(--text-medium)' }}>{payload[0].payload.date}</span>
      </div>
    );
  }
  return null;
...
export default function MoodChart() {
  ...
  return (
    <div className="card p-4 h-100">
      <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.5rem', color: 'var(--forest)', marginBottom: '0.25rem' }}>
        Mood Journey
      </h3>
      <p style={{ color: 'var(--text-light)', fontSize: '0.85rem', marginBottom: '1.25rem' }}>Your last 10 check-ins</p>

      {moods.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-light)', fontSize: '0.9rem', textAlign: 'center', padding: '2rem' }}>
          <div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
        {Object.entries(moodLabels).reverse().map(([val, emoji]) => (
          <span key={val} style={{ fontSize: '0.78rem', color: 'var(--text-light)' }}>{emoji} {['', 'Sad', 'Stressed', 'Neutral', 'Calm', 'Happy'][val]}</span>
        ))}
      </div>
    </div>
  );
